...
//      1. Obtener una lista de los géneros sin repetir
        ...
        //    const generos = movies.reduce ((accum, current) => {
        ...
        //      console.log("Lista de generos",generos);


//      2. Obtener una lista de los nombres de los directores sin repetir

        //    const namesDirector = movies.reduce ((acumm,current)=> {
              
        //     if (!acumm.includes(current.director)) { 
        //      acumm.push(current.director);
        //     }
        //      return acumm

        //    }, []);

        //    console.log("Nombre de directors",namesDirector)
...
//      3. Obtener una lista con únicamente los nombres de los actores protagonistas de todas las películas, sin repetir
        ...
        //     const protagonistActorsNames = movies.reduce ((acumm, current)=> {
        //     current.protagonists.forEach(protagonists => {
        //       if(!acumm.includes(protagonists.name)) {
        //         acumm.push(protagonists.name);
        //       };
        ...
        //   console.log("Nombre de los actores protagonistas",protagonistActorsNames)
        

      
//      4. Obtener uan lista de clasificaciones (rating) sin repetir
    //    const rating = movies.reduce ((acumm, current) => {
    //           if(!acumm.includes(current.rating)) {
    //             acumm.push(current.rating)
    //           }
    //           return acumm;

    //    }, [])

    //    console.log("Lista de rating:",rating)
...
//      5. Obtener una lista de las películas que tienen una duración entre 2 rangos, por ejemplo las películas que duran entre 100 y 120 mins ( los valores de las duraciones deben ser dinámicos)
    //   function duracionPeliculas(movies, minDuracion, maxDuracion) {
    //     return movies.reduce((acumm, peli) => {
    //         if(peli.duration >= minDuracion && peli.duration <= maxDuracion) {
    //             acumm.push(peli);
    //         }
    ...
    //   let filtroDuracion = duracionPeliculas(movies, 100 ,120);
...
function filterMoviesByYear(movies, startYear, endYear) {
    return movies.filter(movie => movie.releaseYear >= startYear && movie.year <= endYear);
}

let filteredMovies = filterMoviesByYear(movies, 2000 , 2010);
console.log(filteredMovies);

//      8. Obtener una lista de películas con base en el país al que pertenecen
         const paises = (paises) => paises.reduce ((acumm, current)=> 
         acumm.includes(current.country) ? acumm : [...acumm, current.country],[]
        
        
        
        );

        console.log (paises(movies))

        const filterMoviesByCountry = movies.filter((peli, pais)=> {
           if (peli => movies.country === country) {
            peli.push(pais);
        ...
        console.log (filterMoviesByCountry)
